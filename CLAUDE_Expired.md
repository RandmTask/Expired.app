...
#### 10. macOS settings menus must keep a fixed checkmark slot
**Problem**: In a scrolling settings screen, `Menu` rows that swap between `Text` and `Label(..., systemImage: "checkmark")` can jitter, shift alignment, and make the popover feel like it is drifting while the page scrolls.
**Fix**: Use a stable menu row helper with a fixed-width checkmark column and a separate stable value label. Avoid `Label` for selected menu rows when alignment matters.
